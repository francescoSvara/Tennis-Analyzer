...
export async function upsertPlayer(player) {
  ...
  if (existing) {
    ...
    // Inserisce
    const { data, error } = await supabase.from('players_new').insert(playerData).select().single();
...
function extractSetScores(homeScore, awayScore) {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    setData[`set${i}_tb`] =
      homeScore?.[`period${i}TieBreak`] || awayScore?.[`period${i}TieBreak`] || null;
  }
...
async function insertStatistics(matchId, statistics) {
  ...
  console.log(
    `📊 Statistiche salvate per match ${matchId} (${records.length} periodi: ${records
      .map((r) => r.period)
      .join(', ')})`
  );
  if (allStats) {
    console.log(
      `   ALL: Aces ${allStats.p1_aces}-${allStats.p2_aces}, DF ${allStats.p1_double_faults}-${allStats.p2_double_faults}, Points ${allStats.p1_total_points_won}-${allStats.p2_total_points_won}`
    );
  }
...
export async function insertMatch(scrapeData) {
  try {
    ...
    console.log(
      `   Match ID: ${insertedMatch.id}, Stats: ${statsCount}, PowerRankings: ${prCount}, PointByPoint: ${pbpCount}`
    );
...
export async function getMatchCompleteness(matchId) {
  ...
  // Ottieni dati match
  const { data: match } = await supabase.from('matches_new').select('*').eq('id', matchId).single();

  if (!match) return { percentage: 0, fields, missingFields: ['match not found'] };

  // Controlla campi base (NUOVO SCHEMA)
  if (match.player1_id) fields.player1_id = true;
  else missingFields.push('player1_id');
  if (match.player2_id) fields.player2_id = true;
  else missingFields.push('player2_id');
  if (match.tournament_id) fields.tournament_id = true;
  else missingFields.push('tournament_id');
  if (match.round) fields.round = true;
  else missingFields.push('round');
  if (match.match_date) fields.match_date = true;
  else missingFields.push('match_date');
  if (match.status) fields.status = true;
  else missingFields.push('status');
  if (match.winner_code) fields.winner_code = true;
  else missingFields.push('winner_code');
  if (match.sets_player1 !== null) fields.sets_player1 = true;
  else missingFields.push('sets_player1');
  if (match.sets_player2 !== null) fields.sets_player2 = true;
  else missingFields.push('sets_player2');
...
export async function ensureDetectedMatchesTable() {
  ...
  const { error } = await supabase.from('detected_matches').select('id').limit(1);
...
export async function syncAcquiredMatches() {
  try {
    ...
    const { data: matchIds, error: matchError } = await supabase.from('matches_new').select('id');
...
export async function findMatchingXlsxMatch(sofascoreMatch) {
  ...
  for (const xlsx of xlsxMatches) {
    ...
    const homeMatchesWinner =
      homeNormalized.includes(xlsxWinner) || xlsxWinner.includes(homeNormalized);
    const homeMatchesLoser =
      homeNormalized.includes(xlsxLoser) || xlsxLoser.includes(homeNormalized);
    const awayMatchesWinner =
      awayNormalized.includes(xlsxWinner) || xlsxWinner.includes(awayNormalized);
    const awayMatchesLoser =
      awayNormalized.includes(xlsxLoser) || xlsxLoser.includes(awayNormalized);

    if ((homeMatchesWinner && awayMatchesLoser) || (homeMatchesLoser && awayMatchesWinner)) {
      console.log(
        `🔗 Match trovato! Sofascore "${homeName} vs ${awayName}" = xlsx "${xlsx.winner_name} vs ${xlsx.loser_name}"`
      );
      return xlsx;
    }
  }
...
export async function mergeXlsxData(sofascoreMatchId, xlsxMatch) {
  ...
  console.log(
    `✅ Match ${sofascoreMatchId} arricchito con dati xlsx (${
...
export async function deleteXlsxMatch(xlsxMatchId) {
  const { error } = await supabase.from('matches_new').delete().eq('id', xlsxMatchId);
...
export async function insertPowerRankingsSvg(matchId, powerRankings) {
  ...
  console.log(
    `📊 [SVG] Inserimento ${powerRankings.length} power rankings per match ${matchIdInt}`
  );
  ...
  for (const pr of powerRankings) {
    ...
    if (existing) {
      ...
      const { error } = await supabase.from('match_power_rankings_new').insert({
        match_id: matchIdInt,
        set_number: setNum,
        game_number: gameNum,
        value: value,
...
export async function insertPowerRankingsMainTable(sofascoreId, powerRankings) {
  ...
  console.log(
    `📊 [SVG-MAIN] Inserimento ${powerRankings.length} power rankings in tabella principale per sofascoreId ${matchIdInt}`
  );

  // Prima elimina eventuali record esistenti per evitare duplicati
  await supabase.from('power_rankings').delete().eq('match_id', matchIdInt);
  ...
  // Inserisci tutti i record
  const { data, error } = await supabase.from('power_rankings').insert(records).select();
...
export async function getPendingMatches(limit = 50) {
  ...
  const enriched = await Promise.all(
    (data || []).map(async (match) => {
      const [p1Res, p2Res, tournRes] = await Promise.all([
        match.player1_id
          ? supabase.from('players_new').select('name').eq('id', match.player1_id).single()
          : { data: null },
        match.player2_id
          ? supabase.from('players_new').select('name').eq('id', match.player2_id).single()
          : { data: null },
        match.tournament_id
          ? supabase.from('tournaments_new').select('name').eq('id', match.tournament_id).single()
          : { data: null },
      ]);
