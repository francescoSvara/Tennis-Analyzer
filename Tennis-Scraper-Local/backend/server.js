...
app.post('/api/mass-scan', async (req, res) => {
  try {
    ...
    console.log(
      `✅ [MASS SCAN] Completato: ${results.tournamentsScanned} tornei, ${results.totalEventsDetected} eventi`
    );
    res.json(results);
  } catch (err) {
...
app.post('/api/match/:eventId/momentum-svg', async (req, res) => {
  ...
  console.log(
    `📊 [SVG MOMENTUM] Richiesta - SofaScore ID: ${sofascoreEventId}, Local ID: ${localId}, dryRun=${!!dryRun}`
  );
  ...
  try {
    ...
    console.log(
      `✅ [SVG MOMENTUM] Salvati ${insertedCountNew} in match_power_rankings_new (ID: ${localId})`
    );

    // Salva ANCHE in power_rankings (tabella per app React) con sofascoreEventId
    let insertedCountMain = 0;
    try {
      insertedCountMain = await insertPowerRankingsMainTable(
        sofascoreEventId,
        result.powerRankings
      );
      console.log(
        `✅ [SVG MOMENTUM] Salvati ${insertedCountMain} in power_rankings (SofaScore ID: ${sofascoreEventId})`
      );
    } catch (mainErr) {
...
app.post('/api/match/:localId/pbp', async (req, res) => {
  ...
  try {
    ...
    console.log(
      `📋 [PBP] Estrazione point-by-point per match ${localId} (SofaScore: ${sofascoreId})`
    );
    ...
    console.log(
      `✅ [PBP] Estratti: ${basicResult.points?.length || 0} score patterns, ${
        detailedResult.totalPoints || 0
      } punti dettagliati`
    );
    ...
    const insertedCount = await insertPointByPointData(
      sofascoreId,
      detailedResult.points || basicResult.points || []
    );
...
app.post('/api/match/:localId/enrich', async (req, res) => {
  ...
  try {
    ...
    const enrichedGames = svgData.map((g) => {
      ...
      const serverNum = awayServesFirstInSet ? (isOddGame ? 2 : 1) : isOddGame ? 1 : 2;
...
async function insertPointByPointData(matchId, points) {
  ...
  for (const point of points) {
    const { error } = await supabase.from('point_by_point').insert({
      match_id: matchId,
...
async function saveEnrichedMatchData(matchId, games) {
  ...
  for (const game of games) {
    ...
    const { error: prError } = await supabase.from('match_power_rankings_new').upsert(
      {
        match_id: matchId,
        set_number: game.set,
    ...
    if (game.points && game.points.length > 0) {
      for (let i = 0; i < game.points.length; i++) {
        ...
        await supabase.from('point_by_point').upsert(
          {
            match_id: matchId,
            set_number: game.set,
            game_number: game.game,
            ...
            away_score: point.awayScore,
            point_winner:
              point.pointWinner === 'home' ? 1 : point.pointWinner === 'away' ? 2 : null,
...
app.get('/api/find-sofascore-id/:localMatchId', async (req, res) => {
  ...
  try {
    ...
    const matchingDetected = detected.find((d) => {
      const homeMatch =
        d.home_team_name?.toLowerCase().includes(player1Name.toLowerCase()) ||
        d.home_team_name?.toLowerCase().includes(player2Name.toLowerCase());
      const awayMatch =
        d.away_team_name?.toLowerCase().includes(player1Name.toLowerCase()) ||
...
app.post('/api/batch-scrape', async (req, res) => {
  ...
  try {
    ...
    const filteredMatches = allMatches.filter((m) => {
      ...
      if (
        tournamentName.includes('M15') ||
        tournamentName.includes('W15') ||
        tournamentName.includes('M25') ||
        tournamentName.includes('W25')
      ) {
        return false;
      }
      return true;
