...
async function addRefreshCountColumns() {
  ...
  if (sample && sample.length > 0) {
    ...
    if (columns.includes('refresh_count')) {
      ...
      console.log('⚠️ refresh_count NON esiste - devi aggiungerla manualmente in Supabase Dashboard');
    }
    
    if (columns.includes('force_completed')) {
      console.log('✅ force_completed già esiste');
    } else {
      console.log('⚠️ force_completed NON esiste - devi aggiungerla manualmente in Supabase Dashboard');
    }
  }
