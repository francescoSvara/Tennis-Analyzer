      ...
      setGames.forEach((game, idx) => {
        ...
        const pointsWithSides = game.points.map((p, pIdx) => {
          ...
          if (server === 'home') {
            ...
            pointWinner = p.winner === 'server' ? 'home' : (p.winner === 'receiver' ? 'away' : null);
          } else {
            homeScore = p.receiverScore;
            awayScore = p.serverScore;
            pointWinner = p.winner === 'receiver' ? 'home' : (p.winner === 'server' ? 'away' : null);
          }
...
function extractPointsFromBlock(pointsHtml) {
  ...
  while ((pointMatch = pointPattern.exec(pointsHtml)) !== null) {
    ...
    const receiverWon = receiverColor.includes('lv1') || receiverColor.includes('secondary-default');
