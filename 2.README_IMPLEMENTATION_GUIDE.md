...
function calculateElasticity(tennisPowerRankings) {
  ...
  const avgRecoveryLength =
    negativePhases.reduce((a, b) => a + b.length, 0) / negativePhases.length;
...
function calculateVolatility(tennisPowerRankings) {
  ...
  for (let i = 1; i < tennisPowerRankings.length; i++) {
    const delta = Math.abs(tennisPowerRankings[i].value - tennisPowerRankings[i - 1].value);
    deltas.push(delta);
  }
...
const MATCH_CARD_WEIGHTS = {
  ...
  surface_stats: 0.2, // Performance su superficie
  tournament_history: 0.1, // Storico in questo torneo
...
const PROFILE_WEIGHTS = {
  career_stats: 0.2,
  current_season: 0.3, // Peso maggiore stagione corrente
  last_3_months: 0.25,
...
| `docs/filosofie/FILOSOFIA_STATS.md`       | Sistema statistiche              |
