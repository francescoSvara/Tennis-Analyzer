    ...
    Object.keys(gamesBySet)
      ...
      .forEach((setNum) => {
        ...
        setGames.forEach((game, idx) => {
          ...
          if (needsFinalPoint) {
            const finalPoint = generateFinalPoint(
              pointsWithSides,
              game.isTiebreak,
              gameWinner,
              server
...
function extractPointsFromBlock(pointsHtml) {
  ...
  while ((pointMatch = pointPattern.exec(pointsHtml)) !== null) {
    ...
    const serverWon =
      serverColor.includes('lv1') ||
      serverColor.includes('secondary-default') ||
      serverColor.includes('status-val');
    const receiverWon =
      receiverColor.includes('lv1') ||
      receiverColor.includes('secondary-default') ||
      receiverColor.includes('status-val');
...
function generateFinalPoint(points, isTiebreak, gameWinner, server) {
  ...
  if (isTiebreak) {
    ...
    // Chi vince il game?
    const serverWins =
      (lastServer === 'home' && gameWinner === 'home') ||
    ...
    return {
      ...
      homeScore:
        lastServer === 'home'
          ? serverWins
            ? 'Game'
            : lastPoint.serverScore
          : serverWins
          ? lastPoint.receiverScore
          : 'Game',
      awayScore:
        lastServer === 'away'
          ? serverWins
  ...
  for (const game of extracted.games) {
    ...
    for (const point of game.points) {
      dbPoints.push({
        ...
        point_winner:
          point.pointWinner === 'home'
            ? 1
            : point.pointWinner === 'away'
            ? 2
            : point.pointWinner
            ? 1
            : 2, // Default a 1 se null
