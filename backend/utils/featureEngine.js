...
function isClutchPoint(matchState = {}) {
  ...
  let homeSetsWon = 0;
  let awaySetsWon = 0;
  for (const set of sets.slice(0, -1)) { // Exclude current set
  ...
  if (isTiebreak) {
    ...
    if (tbHome >= 6 || tbAway >= 6) {
      ...
      if (diff >= 1) { // Someone has set point in tiebreak
  ...
  if (serverScore >= 40 && receiverScore < 40) {
    ...
    if (serverGames >= 5 && serverGames > receiverGames) {
      ...
      if (serverSets >= 1) { // In best of 3, one set won means match point
  ...
  if (homeGame === 50 || awayGame === 50) { // 50 represents AD
    result.isClutchPoint = true;
    result.clutchType = serving === 'home' ? 
      (homeGame === 50 ? 'game_point' : 'break_point') :
      (awayGame === 50 ? 'game_point' : 'break_point');
    result.pressure = 70;
    return result;
  }
