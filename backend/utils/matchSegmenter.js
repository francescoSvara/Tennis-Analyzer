...
function segmentMatch(matchData) {
  ...
  for (let i = 0; i < powerRankings.length; i++) {
    ...
    if (firstBreakFound && segmentation.segments.POST_FIRST_BREAK?.break_holder_performance === 'IN_PROGRESS') {
  ...
  // === FINALIZE SUMMARY ===
  segmentation.summary.avg_break_response_games = breakResponseGames.length > 0
    ? breakResponseGames.reduce((a, b) => a + b, 0) / breakResponseGames.length
    : 0;
  
  segmentation.summary.set_closing_efficiency = setClosingAttempts > 0
    ? setClosingSuccesses / setClosingAttempts
    : 0;
...
function analyzeCriticalGames(matchData) {
  ...
  const avgMomentum = criticalGames.reduce((sum, g) => sum + g.momentum_avg, 0) / criticalGames.length;
...
function analyzeMomentumShifts(matchData) {
  ...
  return {
    ...
    analysis: {
      ...
      net_direction: toHome.length > toAway.length ? 'FAVORS_HOME' : 
                     toAway.length > toHome.length ? 'FAVORS_AWAY' : 'BALANCED',
      match_stability: shifts.length < 3 ? 'STABLE' : 
                       shifts.length < 6 ? 'MODERATE' : 'VOLATILE'
    }
  };
}
