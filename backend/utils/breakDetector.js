...
function detectBreaksFromScore(setScore, firstServer = 'home') {
  ...
  if (!isTiebreak) {
    ...
    if (diff === 2) {
      ...
      if (winner === 'home') { homeBreaks = 1; awayBreaks = 0; }
      else { awayBreaks = 1; homeBreaks = 0; }
    } else if (diff === 3) {
      // 6-3: 1-2 breaks typically
      if (winner === 'home') { homeBreaks = 2; awayBreaks = 1; }
      else { awayBreaks = 2; homeBreaks = 1; }
    } else if (diff === 4) {
      // 6-2: 2 breaks with 0 or 2 with 0
      if (winner === 'home') { homeBreaks = 2; awayBreaks = 0; }
      else { awayBreaks = 2; homeBreaks = 0; }
    } else if (diff === 5) {
...
function analyzeSetBreaks(matchData) {
  ...
  const avgBreaksPerSet = sets.length > 0 
    ? (totalHomeBreaks + totalAwayBreaks) / sets.length 
    : 0;
...
function classifySet(setAnalysis) {
  ...
  return {
    ...
    dominant_player: isOneSided 
      ? (home_breaks > away_breaks ? 'home' : 'away') 
      : null
  };
}
