...
function extractPlayersFromHtml(html) {
  ...
  const playerLinkPattern = /<a[^>]*href="\/[^"]*\/player\/([^"\/]+)\/(\d+)"[^>]*>[\s\S]*?<span[^>]*>([^<]+)<\/span>/gi;
...
function extractRawPointsFromBlock(pointsHtml) {
  ...
  const columnPattern = /<div[^>]*class="[^"]*d_flex[^"]*flex-d_column[^"]*gap_sm[^"]*"[^>]*>([\s\S]*?)<\/div>/gi;
  ...
  while ((match = columnPattern.exec(pointsHtml)) !== null) {
    ...
    const spanPattern = /<span[^>]*style="color:\s*var\(--colors-([^)]+)\)[^"]*"[^>]*>([^<]*)<\/span>/gi;
    const spans = [];
    let spanMatch;
...
function resolveSemantics(rawData, context) {
  ...
  const setIndices = Object.keys(blocksBySet).map(k => parseInt(k)).sort((a,b) => a - b);
  ...
  if (firstSetIdx && blocksBySet[firstSetIdx]) {
    ...
    if (game1Block) {
      ...
      if (game1Block.serverSlug) {
        const slugRow = game1Block.serverSlug === players.row1?.slug ? 'row1' : 
                        game1Block.serverSlug === players.row2?.slug ? 'row2' : null;
        if (slugRow) {
    ...
    for (let gameIdx = 0; gameIdx < orderedBlocks.length; gameIdx++) {
      ...
      if (currentSetFirstServer) {
        if (isTiebreak) {
          ...
          serverSide = (gameNumber % 2 === 1) ? currentSetFirstServer : 
            (currentSetFirstServer === 'home' ? 'away' : 'home');
        }
      }
