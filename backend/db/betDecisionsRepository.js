...
async function insertDecision(decision) {
  ...
  const { data, error } = await supabase.from('bet_decisions').insert(record).select().single();
  ...
  console.log(
    `✅ Bet decision logged: match=${decision.matchId} market=${decision.market} decision=${decision.decision}`
  );
  return data;
}
