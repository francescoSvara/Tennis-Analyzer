...
async function deduplicateAll() {
  ...
  for (const group of duplicateGroups) {
    ...
    console.log(`   ⭐ Tengo: ID ${best.match.id} (${best.match.data_source}, quality: ${best.quality})`);
    
    // Mostra quali verranno eliminati
    for (const dup of toDelete) {
      console.log(`   🗑️  Elimino: ID ${dup.match.id} (${dup.match.data_source}, quality: ${dup.quality})`);
    }
    ...
    for (const dup of toDelete) {
      ...
      if (m.raw_json) {
        ...
        if (dupRaw.statistics && !bestRaw.statistics) {
          mergeData.raw_json = { ...(mergeData.raw_json || bestRaw), statistics: dupRaw.statistics };
        }
        if (dupRaw.pointByPoint && !bestRaw.pointByPoint) {
          mergeData.raw_json = { ...(mergeData.raw_json || bestRaw), pointByPoint: dupRaw.pointByPoint };
        }
        ...
        if (dupRaw.xlsx_original_id || m.data_source?.includes('xlsx')) {
          ...
          mergeData.raw_json = { ...existingRaw, xlsx_original_id: dupRaw.xlsx_original_id || m.id };
        }
      }
    }
