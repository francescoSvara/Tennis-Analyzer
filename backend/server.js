...
const { analyzePowerRankings } = require('./utils/valueInterpreter');
const {
  initLiveManager,
  fetchCompleteData,
  startScheduler,
...
try {
  ...
  console.log('âœ… Player Stats Service loaded');
} catch (e) {
  console.warn('âš ï¸ Player Stats Service not available:', e.message);
}
...
try {
  ...
  console.log('âœ… Match Card Service loaded');
} catch (e) {
  console.warn('âš ï¸ Match Card Service not available:', e.message);
}
...
try {
  ...
  console.log('âœ… Player Service loaded');
} catch (e) {
  console.warn('âš ï¸ Player Service not available:', e.message);
}
...
try {
  ...
  console.log('âœ… Player Profile Service loaded');
} catch (e) {
  console.warn('âš ï¸ Player Profile Service not available:', e.message);
}
...
try {
  ...
  console.log('âœ… Break Detector loaded');
} catch (e) {
  console.warn('âš ï¸ Break Detector not available:', e.message);
}
...
try {
  ...
  console.log('âœ… Pressure Calculator loaded');
} catch (e) {
  console.warn('âš ï¸ Pressure Calculator not available:', e.message);
}
...
try {
  ...
  console.log('âœ… Match Segmenter loaded');
} catch (e) {
  console.warn('âš ï¸ Match Segmenter not available:', e.message);
}
...
// ============================================================================
// Cache per bundle giÃ  costruiti - evita ricostruzione continua
 ...
 * @see docs/comments/server-js-explanations.md#calculateBreaksFromPbp
 */
 ...
 * @see docs/comments/server-js-explanations.md#generatePowerRankingsFromPbp
 */
function generatePowerRankingsFromPbp(pointByPoint) {
  ...
  for (const setData of pointByPoint) {
    ...
    for (const gameData of setData.games || []) {
      ...
      // Determina chi ha vinto il game e se Ã¨ break
      ...
      // Break = chi vince NON Ã¨ chi serve
 ...
 * Se powerRankings Ã¨ vuoto ma abbiamo pointByPoint, genera powerRankings simulati
 *
...
function enrichPowerRankingsWithBreaks(powerRankings, pointByPoint) {
  ...
  if ((!powerRankings || powerRankings.length === 0) && pointByPoint?.length > 0) {
    ...
    if (generated.length > 0) {
      console.log(`   ðŸ“Š Generated ${generated.length} powerRankings from pointByPoint`);
      return generated;
    }
    return [];
  }
...
function calculateDataCompleteness(apiData, eventId, sport = 'tennis') {
  ...
  if (isTennis) {
    ...
    maxScore += 35;
    const hasEvent = Object.keys(apiData).some(
      (k) => k.match(/\/event\/\d+$/) && apiData[k]?.event
    );
    ...
    // Statistics (20 punti)
    maxScore += 20;
    const statsKey = Object.keys(apiData).find(
      (k) => k.includes('/statistics') && !k.includes('team-statistics')
    );
    ...
    if (
      (h2hData && (h2hData.teamDuel || h2hData.managerDuel)) ||
      (tpData && tpData.tennisPowerRankings)
    ) {
      score += 10;
    }
  ...
  const checks = [
    {
      weight: 25,
      check: () =>
        Object.keys(apiData).some((k) => k.match(/\/event\/\d+$/) && apiData[k]?.event) ? 1 : 0,
    },
    {
      weight: 20,
      check: () => (Object.keys(apiData).some((k) => k.includes('/statistics')) ? 1 : 0.5),
    },
    {
      weight: 15,
      check: () => (Object.keys(apiData).some((k) => k.includes('/incidents')) ? 1 : 0.5),
    },
    { weight: 15, check: () => (Object.keys(apiData).some((k) => k.includes('/h2h')) ? 1 : 0.5) },
    {
      weight: 15,
      check: () => (Object.keys(apiData).some((k) => k.includes('/lineups')) ? 1 : 0.5),
    },
...
app.get('/api/match/:eventId', async (req, res) => {
  ...
  try {
    ...
    if (matchRepository) {
      try {
        ...
        if (dbMatch && !forceRefresh) {
          ...
          console.log(
            `   PowerRankings: ${enrichedPR.length}, Breaks: ${breaksCount}, Statistics: ${
              dbMatch.statistics?.length || 0
            }, PBP: ${dbMatch.pointByPoint?.length || 0}`
          );
          ...
          return res.json({
            ...
            // Aggiungi alias per compatibilitÃ  frontend
    ...
    if (breaksCount > 0) {
      console.log(`   ðŸŽ¾ Break calcolati da PBP: ${breaksCount}`);
    }
    ...
    // Aggiungi alias tennisPowerRankings per compatibilitÃ  frontend
    res.json({
      source: 'sofascore',
      eventId,
      ...liveData,
 ...
 * @see docs/comments/server-js-explanations.md#get-apimatcheventidbundle
 */
app.get('/api/match/:eventId/bundle', async (req, res) => {
  ...
  try {
    ...
    if (cachedBundle) {
      console.log(`ðŸ“¦ Bundle for ${eventId} served from cache`);
      return res.json(cachedBundle);
    }

    console.log(`ðŸ“¦ Building bundle for match ${eventId}...`);
    ...
    // ============================================================
    // FILOSOFIA: Se match non nel DB, SYNC da SofaScore â†’ DB â†’ Serve
    // ============================================================
    if (!finalMatchData) {
      console.log(`ðŸ“¦ Match ${eventId} not in DB, syncing from SofaScore...`);
      try {
        // Usa syncMatch per fetch + save in DB
        const synced = await syncMatch(parseInt(eventId));
        if (synced?.success) {
          console.log(`âœ… Synced match ${eventId} from SofaScore`);
          // Ricarica dati dal DB dopo sync
          [matchData, statisticsData, momentumData, pointsData, matchScoresData] =
            await Promise.all([
              matchCardService
                ? matchCardService.getMatchCardFromSnapshot(parseInt(eventId))
                : null,
              matchRepository ? matchRepository.getMatchStatisticsNew(parseInt(eventId)) : [],
              matchRepository ? matchRepository.getMatchMomentum(parseInt(eventId)) : [],
              matchRepository
        ...
        console.warn(`âš ï¸ Sync failed for ${eventId}:`, syncErr.message);
      }
    }

    // ============================================================
    // FILOSOFIA: Se dati incompleti (surface=Unknown, quality<50),
    // SYNC da SofaScore â†’ DB â†’ Rileggi
    // ============================================================
    const needsRefresh =
      finalMatchData &&
    ...
    if (needsRefresh) {
      console.log(
        `ðŸ”„ Match ${eventId} has incomplete data (surface=${finalMatchData.match?.surface}, quality=${finalMatchData.dataQuality}%), refreshing from SofaScore...`
      );
      try {
        const synced = await syncMatch(parseInt(eventId));
        if (synced?.success) {
          console.log(`âœ… Refreshed match ${eventId} from SofaScore`);
        ...
        console.warn(`âš ï¸ Refresh failed for ${eventId}:`, refreshErr.message);
    ...
    if (
      ...
      console.log(
        `ðŸ“Š Enriching match ${eventId} with ${setScores.length} set scores from match_scores table`
      );
    ...
    // Point-by-point: SEMPRE dal DB (filosofia: SofaScore â†’ DB â†’ Frontend)
    // Se non ci sono dati PbP nel DB, il sync li ha giÃ  caricati da SofaScore
    ...
    console.log(`ðŸ“Š Stats source for ${eventId}:`, {
      hasNewStats,
      ...
      finalMatchStatisticsType: Array.isArray(finalMatchData?.statistics)
        ? 'array'
        : typeof finalMatchData?.statistics,
    });
    ...
    console.log(`ðŸ“Š Stats result for ${eventId}:`, {
    ...
    const tabs = {
      ...
      momentum: {
        ...
        // qualityStats usa i valori giÃ  calcolati in statsTab
        qualityStats: {
          home: {
    ...
    try {
      ...
      if (normalizedOdds?.home?.value && features.dominance) {
        const modelProb = features.dominance / 100; // Converti dominance in probabilitÃ 
        riskAnalysis = riskEngine.analyzeRisk({
          modelProb,
          marketOdds: normalizedOdds.home.value,
          bankroll: 1000, // Default, frontend puÃ² ricalcolare
    ...
    try {
      ...
      console.warn(`âš ï¸ Surface splits calculation failed:`, surfaceErr.message);
    }
    ...
    try {
      ...
      if (qualityResult.status === 'FAIL') {
        console.warn(
          `âš ï¸ Bundle quality FAIL for ${eventId}:`,
          qualityResult.issues.map((i) => i.code).join(', ')
        );
      }
    } catch (qualityErr) {
      console.warn(`âš ï¸ Quality check failed:`, qualityErr.message);
    }
    ...
    if (process.env.LOG_BET_DECISIONS === 'true' && riskAnalysis?.recommendation) {
      try {
        ...
        console.warn(`âš ï¸ Bet decision logging failed:`, betLogErr.message);
      }
    }
    ...
    const isLive =
      bundleMatchStatus === 'inprogress' ||
      bundleMatchStatus === 'live' ||
      bundleMatchStatus === 'playing';
    setCachedBundle(eventId, bundle, isLive);

    console.log(
      `âœ… Bundle built for ${eventId}: quality=${dataQuality}%, strategies=${strategySignals.length} (${strategySummary.ready} ready)`
    );
    res.json(bundle);
  } catch (err) {
...
function normalizeOddsForBundle(oddsData, matchData) {
  ...
  if (matchData?.odds?.matchWinner) {
    ...
    // Se giÃ  nel formato corretto
    if (mw.home?.value !== undefined) {
      return mw;
    }
    // Se Ã¨ un oggetto semplice { home: number, away: number }
...
function normalizePointsForBundle(pointsData) {
  ...
  for (const [_, gameData] of gameMap) {
    ...
    // serving=1 â†’ home, serving=2 â†’ away
    let server = 'unknown';
    ...
    // scoring=1 â†’ home vince, scoring=2 â†’ away vince
    // BREAK = serving !== scoring (chi serve perde)
    let gameWinner = null;

    // Cerca scoring in qualsiasi punto del game (di solito Ã¨ nell'ultimo)
  ...
  const points = pointsData.data.map((p) => {
    ...
    // Nel tennis il punteggio Ã¨ normalmente server-receiver, ma per chiarezza UI
    // lo mostriamo sempre come HOME-AWAY (l'ordine dei giocatori nell'header)
    // PRIORITÃ€: home_point/away_point > score_p1/score_p2 > score_after > score_before
    let score = '';
...
function buildStatsTab(statisticsData, matchData, score, pointByPoint = []) {
  // DEBUG: Log input
  console.log('ðŸ“Š buildStatsTab input:', {
  ...
  const convertSofaScoreArrayFormat = (statsArray) => {
    ...
    for (const periodData of statsArray) {
        ...
        for (const item of group.statisticsItems) {
          ...
          switch (key) {
            ...
            case 'secondServeAccuracy':
              ...
              // In tennis tutte le seconde "vanno dentro" (altrimenti Ã¨ doppio fallo)
            ...
            case 'breakPointsScored':
              ...
              // homeValue = BP convertiti (NON c'Ã¨ homeTotal in SofaScore per questo!)
              stats.home.breakPointsWon = item.homeValue || 0;
              stats.away.breakPointsWon = item.awayValue || 0;
              // breakPointsTotal viene calcolato dopo: Ã¨ uguale a breakPointsFaced dell'avversario
              break;
            case 'pointsTotal':
      ...
      // FILOSOFIA: Calcola totalPointsWon per i SET quando pointsTotal non Ã¨ disponibile
      ...
      // HOME ha avuto X opportunitÃ  di break = AWAY ha affrontato X break points
      ...
      if (stats.home.receiverPointsWon !== undefined && stats.away.servicePointsWon !== undefined) {
        const homeTotalReceiverPoints =
          (stats.away.servicePointsWon || 0) + (stats.home.receiverPointsWon || 0);
        stats.home.returnPointsWonPct =
          homeTotalReceiverPoints > 0
      ...
      if (stats.away.receiverPointsWon !== undefined && stats.home.servicePointsWon !== undefined) {
        const awayTotalReceiverPoints =
          (stats.home.servicePointsWon || 0) + (stats.away.receiverPointsWon || 0);
        stats.away.returnPointsWonPct =
          awayTotalReceiverPoints > 0
  ...
  // Se Ã¨ formato array SofaScore, convertilo prima
  if (Array.isArray(statisticsData)) {
    statisticsData = convertSofaScoreArrayFormat(statisticsData);
  }

  // Rileva se Ã¨ il nuovo formato (byPeriod) o il vecchio
  ...
  const extractStatsFromRecord = (record) => {
    ...
    // Se il record ha giÃ  la struttura home/away (convertito da SofaScore), usala direttamente
  ...
  // Estrai stats dal PBP se disponibile - QUESTA Ãˆ LA FONTE PRIMARIA
  // pointByPoint puÃ² essere: array (legacy) o oggetto { points: [...] }
  ...
  const buildTabFromStats = (stats) => {
    // MERGE: Se stats DB Ã¨ undefined/null, usa pbpStats se disponibile
    // ATTENZIONE: 0 Ã¨ un valore VALIDO, non usare fallback per 0!
    ...
    // Winners â‰ˆ aces + 15% punti vinti primo servizio + 10% punti vinti in risposta
    ...
    if ((!homeWinners || homeWinners === 0) && homeTotalWon > 0) {
      ...
      // Winners â‰ˆ aces + 15% dei punti vinti al primo servizio (colpi vincenti)
    ...
    // Formula: UE â‰ˆ doubleFaults + 12% dei punti persi (errori non forzati)
    ...
    if ((!homeUE || homeUE === 0) && homePointsLost > 0) {
      ...
      // UE â‰ˆ doubleFaults + 12% dei punti persi
    ...
    // Semplificazione: returnPct â‰ˆ 100 - avversario1stServeWonPct (se serve solo prima)
    ...
    if ((!homeReturnPct || homeReturnPct === 0) && awayTotalWon > 0) {
      ...
      // returnPct â‰ˆ 100 - weighted average of away's serve win %
      ...
      // Weighted: (1st% Ã— 1stWon%) + ((100-1st%) Ã— 2ndWon%)
      const awayServeWonPct =
        (away1stPct * away1stWonPct + (100 - away1stPct) * away2ndWonPct) / 100;
    ...
    if ((!awayReturnPct || awayReturnPct === 0) && homeTotalWon > 0) {
      ...
      const homeServeWonPct =
        (home1stPct * home1stWonPct + (100 - home1stPct) * home2ndWonPct) / 100;
    ...
    if (!homeFirstServeWonPct || homeFirstServeWonPct === 0) {
      // Stima basata su punti vinti: se vinci piÃ¹ punti, serve meglio
    ...
    if (homeFirstServeTotal === 0 && homeFirstServePct > 0 && homeTotalWon > 0) {
      // Stima: totalServes â‰ˆ totalPointsWon * 1.2 (circa)
    ...
    // ===== BREAK POINTS - USA pbpStats (giÃ  calcolato sopra) =====
    ...
    // Calcola breakPointsTotal per return (quante opportunitÃ  hai avuto in attacco)
    ...
    // Log per debug
    console.log(
      'ðŸŽ¾ Break Points Final:',
      JSON.stringify({
        home: {
          faced: homeBreakPointsFaced,
          saved: homeBreakPointsSaved,
          won: homeBreakPointsWon,
          total: homeBreakPointsTotal,
        },
        away: {
          faced: awayBreakPointsFaced,
          saved: awayBreakPointsSaved,
          won: awayBreakPointsWon,
          total: awayBreakPointsTotal,
        },
      })
    );

    // Se breakPointsTotal=0 ma breakPointsWon>0, il total Ã¨ almeno il won
    ...
    // HOME faced = quante opportunitÃ  ha avuto AWAY = AWAY breakPointsTotal
    ...
    if (
      homeServicePointsWon === 0 &&
      (homeFirstServePointsWon > 0 || homeSecondServePointsWon > 0)
    ) {
      homeServicePointsWon = homeFirstServePointsWon + homeSecondServePointsWon;
    }
    if (
      awayServicePointsWon === 0 &&
      (awayFirstServePointsWon > 0 || awaySecondServePointsWon > 0)
    ) {
    ...
    if (
      homeSecondReturnPointsWon === 0 &&
      awaySecondServeTotal > 0 &&
      awaySecondServePointsWon > 0
    ) {
      homeSecondReturnPointsWon = awaySecondServeTotal - awaySecondServePointsWon;
      homeSecondReturnPointsTotal = awaySecondServeTotal;
    }
    if (
      awaySecondReturnPointsWon === 0 &&
      homeSecondServeTotal > 0 &&
      homeSecondServePointsWon > 0
    ) {
    ...
    // Questo Ã¨ piÃ¹ accurato di total - service perchÃ© usa i dati grezzi
    ...
    // Se servicePointsWon + returnPointsWon != totalWon, c'Ã¨ un errore
    ...
    if (homeTotalWon > 0 && Math.abs(homeSumCheck - homeTotalWon) > 2) {
      // C'Ã¨ discrepanza, usa total - return per service
  ...
  if (isNewFormat) {
    ...
    console.log(
      'ðŸŽ¯ Game Stats from SofaScore:',
      JSON.stringify({
        home: {
          gamesWon: allStats.home?.gamesWon,
          tiebreaks: allStats.home?.tiebreaksWon,
          consec: allStats.home?.maxConsecutiveGamesWon,
          serviceWon: allStats.home?.serviceGamesWon,
        },
        away: {
          gamesWon: allStats.away?.gamesWon,
          tiebreaks: allStats.away?.tiebreaksWon,
          consec: allStats.away?.maxConsecutiveGamesWon,
          serviceWon: allStats.away?.serviceGamesWon,
        },
      })
    );
  ...
  let homeBreaksConverted;
  let awayBreaksConverted;
  ...
  homeBreaksConverted = Math.max(0, awayGames - awaySetsWon * 3); // rough estimate
  awayBreaksConverted = Math.max(0, homeGames - homeSetsWon * 3);
 ...
 * FILOSOFIA: DATA_LAYER â†’ estrae dati dallo score
...
function calculateGameStatsFromScore(score) {
  ...
  for (const set of sets) {
    ...
    // Tiebreak: se il set finisce 7-6 o 6-7, c'Ã¨ stato un tiebreak
    // Il vincitore del tiebreak Ã¨ chi ha vinto il set
  ...
  for (const set of sets) {
    ...
    // diff >= 4 â†’ vincitore ha ~3-4 consecutive, diff >= 2 â†’ vincitore ha ~2-3
    if (homeSetGames > awaySetGames) {
      // Home vince set
      homeMaxConsecutive = Math.max(homeMaxConsecutive, Math.min(diff + 1, 5));
      // Away perde ma avrÃ  tenuto almeno 1-2 servizi consecutivi
      ...
      // Home perde ma avrÃ  tenuto almeno 1-2 servizi consecutivi
 ...
 * Break point = receiver Ã¨ a 40/A mentre server non ha A
 * Se server=away e homeScore=40 â†’ Home (receiver) ha break point
 * Se server=home e awayScore=40 â†’ Away (receiver) ha break point
 */
/**
 * FUNZIONE MASTER: Estrae TUTTE le statistiche dal PBP in un JSON strutturato
 * Questa Ã¨ la SINGOLA fonte di veritÃ  per tutti i calcoli statistici
 * Score format: "home-away" (es. "40-30" = Home ha 40, Away ha 30)
 */
function extractAllStatsFromPBP(points) {
  ...
  for (const point of points) {
    ...
    if (winner === 'home') {
      ...
      stats.home.maxConsecutivePointsWon = Math.max(
        stats.home.maxConsecutivePointsWon,
        currentConsecutiveHome
      );
      ...
      stats.away.maxConsecutivePointsWon = Math.max(
        stats.away.maxConsecutivePointsWon,
        currentConsecutiveAway
      );
    ...
    if (server === 'home') {
      ...
      if (!isDoubleFault) {
        // Se non Ã¨ doppio fallo, Ã¨ o prima o seconda andata dentro
        // Usiamo una euristica: track dei servizi per game
        currentGameFirstServes.home++;
        if (currentGameFirstServes.home <= 4) {
          // ~60% prime nei primi punti
    ...
    // Ogni punto con score 40-X o A-40 Ã¨ un break point UNICO
    // Non usiamo chiavi per evitare duplicati - ogni punto Ã¨ un BP separato
    const parts = score.split('-');
    if (parts.length === 2) {
      const homeScore = parts[0].trim();
      const awayScore = parts[1].trim();

      // BP per Home (quando Away serve e Home Ã¨ a 40/A)
      if (server === 'away') {
        const isBP =
          (homeScore === '40' && ['0', '15', '30'].includes(awayScore)) ||
          (homeScore === 'A' && awayScore === '40');
        if (isBP) {
          // Ogni punto con questo score Ã¨ un break point
          stats.away.breakPointsFaced++;
          stats.home.breakPointsTotal++;
          if (winner === 'away') {
            stats.away.breakPointsSaved++;
      ...
      // BP per Away (quando Home serve e Away Ã¨ a 40/A)
      if (server === 'home') {
        const isBP =
          (awayScore === '40' && ['0', '15', '30'].includes(homeScore)) ||
      ...
      if (lastGameServer && lastGameWinner) {
        if (lastGameServer === 'home') {
          ...
          if (lastGameWinner === 'home') {
            ...
            stats.home.consecutiveGamesWon = Math.max(
              stats.home.consecutiveGamesWon,
              currentConsecutiveGamesHome
            );
            ...
            stats.away.consecutiveGamesWon = Math.max(
              stats.away.consecutiveGamesWon,
              currentConsecutiveGamesAway
            );
    ...
    lastGameWinner = winner; // L'ultimo winner del game Ã¨ chi ha vinto il game
  }
...
function calculateBreakStatsFromPointByPoint(pointByPoint) {
  ...
  for (const p of pointByPoint) {
    ...
    if (server && winner) {
      if (server === 'home') {
        homeGamesServed++;
        if (winner !== 'home') {
          // Home serve e perde â†’ away converte break
          awayBreakPointsWon++;
        }
      } else {
        awayGamesServed++;
        if (winner !== 'away') {
          // Away serve e perde â†’ home converte break
          homeBreakPointsWon++;
        }
      }
    }
  ...
  // Se ci sono molti break (>70%), stima piÃ¹ break points
  ...
  const homeBreakPointsTotal = Math.max(
    homeBreakPointsWon,
    Math.round(awayGamesServed * homeBpMultiplier)
  );
  const awayBreakPointsTotal = Math.max(
    awayBreakPointsWon,
    Math.round(homeGamesServed * awayBpMultiplier)
  );
...
app.get('/api/match/:eventId/card', async (req, res) => {
  ...
  try {
    ...
    if (useSnapshot === 'true' && matchRepository) {
      ...
      if (snapshot) {
        console.log(
          `âš¡ Match card ${eventId} from snapshot (quality=${snapshot.data_quality_int}%)`
        );
    ...
    console.log(
      `ðŸŽ¾ Match card ${eventId}: quality=${card.dataQuality}%, sources=${card.dataSources?.join(
        ', '
      )}`
    );
...
app.post('/api/match/:eventId/momentum-svg', async (req, res) => {
  ...
  try {
    ...
    // FILOSOFIA: Genera point-by-point sintetici SOLO se non esistono giÃ 
    ...
    if (!existingPbp.data || existingPbp.data.length === 0) {
      ...
      console.log(
        `ðŸ“Š SVG Momentum inserito per match ${eventId}: ${inserted} game, ${pbpInserted} punti sintetici generati`
      );
    } else {
      console.log(
        `ðŸ“Š SVG Momentum inserito per match ${eventId}: ${inserted} game (point_by_point giÃ  esistenti, non sovrascritti)`
      );
    }
...
async function generatePointByPointFromSvg(matchId, rankings) {
  ...
  for (const [setNum, games] of setMap) {
    ...
    for (const g of games) {
      ...
      points.push({
        ...
        away_point: 'G',
        score_before: `${homeGames - (gameWinner === 'home' ? 1 : 0)}-${
          awayGames - (gameWinner === 'away' ? 1 : 0)
        }`,
  ...
  if (points.length > 0) {
    ...
    console.log(
      `Generated ${points.length} synthetic point-by-point records from SVG for match ${matchId}`
    );
  }
...
app.get('/api/matches/cards', async (req, res) => {
  ...
  try {
    const matches = await matchCardService.getRecentMatches(parseInt(limit), {
      surface,
      tournament: tournament ? parseInt(tournament) : null,
  ...
  try {
    ...
    if (playerMatches.length > 0) {
      ...
      if (matchesWithOdds.length > 0) {
        const avgOdds =
          matchesWithOdds.reduce(
            (sum, m) => sum + (parseFloat(m.avgw) || parseFloat(m.avgl) || 0),
            0
      ...
      // Range date
      const dates = playerMatches
        .map((m) => m.dateTimestamp || m.date || m.start_time)
        .filter(Boolean)
        .sort();
        ...
        if (dbMatches && dbMatches.length > 0) {
          const matches = dbMatches.map((m) => ({
            ...
            startTimestamp: m.start_time
              ? Math.floor(new Date(m.start_time).getTime() / 1000)
              : null,
...
/**
 * POST /api/matches/enrich - DEPRECATO (tabella 'matches' rimossa)
 */
app.post('/api/matches/enrich', async (req, res) => {
  res.status(410).json({
    error: 'Endpoint deprecated',
    message: "La tabella 'matches' non esiste piÃ¹. Usare match_card_snapshot.",
  });
});

/**
 * GET /api/matches/missing-data - DEPRECATO (tabella 'matches' rimossa)
 */
app.get('/api/matches/missing-data', async (req, res) => {
  res.status(410).json({
    error: 'Endpoint deprecated',
    message: "La tabella 'matches' non esiste piÃ¹.",
  });
});

/**
 * POST /api/match/:eventId/find-sofascore - DEPRECATO (tabella 'matches' rimossa)
 */
