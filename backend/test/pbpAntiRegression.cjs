...
function test_tiebreakNotAsNormalGame() {
  ...
  assert.strictEqual(
    isTiebreakPoints(tiebreakPoints),
    true,
    'isTiebreakPoints deve riconoscere punti tiebreak'
  );

  assert.strictEqual(
    isTiebreakPoints(normalPoints),
    false,
  ...
  for (const block of rawData.rawBlocks) {
    ...
    for (const point of block.rawPoints) {
      if (!isTiebreak) {
        ...
        assert.ok(s1Valid || s1Numeric, `Score row1 invalido in game normale: ${point.row1Score}`);
        assert.ok(s2Valid || s2Numeric, `Score row2 invalido in game normale: ${point.row2Score}`);
      }
    }
  }
}
...
function test_goldenMatchValidation() {
  ...
  assert.strictEqual(
    set1.finalScore.home,
    GOLDEN_DATA.set1.homeGames,
    `Set 1 home deve avere ${GOLDEN_DATA.set1.homeGames} games`
  );
  assert.strictEqual(
    set2.finalScore.home,
    GOLDEN_DATA.set2.homeGames,
...
function test_codeInspection() {
  ...
  assert.ok(
    !setNumberMatches || setNumberMatches.length === 0,
