...
function loadDependencies() {
  if (!matchRepository) {
    try {
      ...
      console.warn('⚠️ matchRepository not available for stats');
    }
  }
  if (!supabaseClient) {
    try {
      ...
      console.warn('⚠️ supabaseClient not available for stats');
    }
  }
  if (!liveManager) {
    try {
      ...
      console.warn('⚠️ liveManager not available for stats');
    }
  }
}
  ...
  try {
    ...
    for (const match of dbMatches || []) {
      ...
      const isSofascore =
        ...
        (match.raw_json &&
          typeof match.raw_json === 'string' &&
          match.raw_json.includes('sofascore'));
    ...
    const completenessScore =
      totalMatches > 0 ? Math.round((matchesWith100Percent / totalMatches) * 100) : 0;
    const qualityScore =
      totalPossibleFields > 0 ? Math.round((totalFilledFields / totalPossibleFields) * 100) : 0;
    ...
    const powerScore = Math.round(
      dbSizeScore * 0.25 + completenessScore * 0.3 + qualityScore * 0.3 + finishedScore * 0.15
    );
      ...
      summary: {
        ...
        powerDetails: {
          ...
          quality: {
            score: qualityScore,
            label: 'Qualità Dati',
            detail: `${totalFilledFields}/${totalPossibleFields} campi (${
              totalMatches > 0 ? Math.round(totalFilledFields / totalMatches) : 0
            } per match)`,
          },
          finished: {
            score: finishedScore,
            label: 'Match Finiti',
            detail: `${finishedMatches} finiti · ${matchesByStatus.inprogress || 0} live · ${
              matchesByStatus.notstarted || 0
            } da giocare`,
          },
        },
        sources: {
