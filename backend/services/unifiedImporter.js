...
/**
 * Controlla se un match esiste già nel DB 
...
async function findExistingMatch(matchDate, winnerName, loserName) {
  ...
  const { data, error } = await supabase
    ...
    .or(`and(winner_name.eq.${winnerName},loser_name.eq.${loserName}),and(winner_name.eq.${loserName},loser_name.eq.${winnerName})`)
    .limit(1);
  ...
  if (error) {
    ...
    const { data: fallbackData } = await supabase
      ...
      .select('*')
      .eq('match_date', matchDate)
      .or(`winner_name.ilike.%${winnerName.split(' ').pop()}%,loser_name.ilike.%${winnerName.split(' ').pop()}%`)
      .limit(5);
...
async function upsertMatch(matchData, source = 'unknown') {
  ...
  if (error) {
    console.error(`❌ Error upserting match ${matchId}:`, error.message);
    stats.errors++;
    return null;
  }
...
async function importSofascoreJson(filePath, options = {}) {
  ...
  // Struttura Sofascore può variare
