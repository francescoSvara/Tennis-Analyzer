 ...
 * @see docs/comments/player-stats-service-explanations.md#header
 */
...
function extractLastName(name) {
  ...
  // Formato "Cognome I" (xlsx) - prima parte Ã¨ cognome
  if (parts.length >= 2 && parts[parts.length - 1].length <= 2) {
    return parts[0].toLowerCase();
  }

  // Formato "Nome Cognome" - ultima parte Ã¨ cognome
...
async function getPlayerMatches(playerName) {
  ...
  console.log(`ðŸ” [Stats] Searching matches for "${playerName}" (lastName: ${lastName})`);

  // ===== STRATEGY 1: Search by player_id directly (primary strategy for matches) =====
  // Note: matches uses player1_id/player2_id instead of winner_name/loser_name
  // Cerca ID giocatore in players
  ...
  // Skip legacy winner_name/loser_name search as matches doesn't have these columns
  let matchesWinner = [];
  let matchesLoser = [];
  const err1 = null;
  const err2 = null;
  ...
  // ===== STRATEGY 2: Search by player_id in matches =====
  // matches uses player1_id and player2_id columns
  ...
  if (matchingPlayerIds.length > 0) {
    const { data: player1Matches, error: p1Err } = await supabase
      ...
      .select(
        `
        *,
        player1:players!matches_player1_id_fkey(id, name, full_name),
        player2:players!matches_player2_id_fkey(id, name, full_name)
      `
      )
  ...
  // ===== COMBINE ALL MATCHES =====
  const allMatches = [...(matchesWinner || []), ...(matchesLoser || []), ...matchesByPlayerId];
  ...
  for (const match of allMatches) {
    ...
    // Check by player_id if not found by name (matches format)
    if (!isWinner && !isLoser && matchingPlayerIds.length > 0) {
      ...
      if (isPlayer1 || isPlayer2) {
        ...
        if (winnerCode === 1 && isPlayer1) isWinner = true;
          ...
          // Fallback: check sets (matches uses sets_player1/sets_player2)
    ...
    // Check player1/player2 objects from join (matches format)
    if (!isWinner && !isLoser) {
  ...
  console.log(
    `   âœ… [Stats] Total matches: ${result.length} (${
      result.filter((m) => m.player_role === 'winner').length
    }W - ${result.filter((m) => m.player_role === 'loser').length}L)`
  );
...
function calculateComebackRate(matches, playerName) {
  ...
  for (const match of matches) {
    ...
    // Se il player Ã¨ winner e l1 > w1 â†’ ha perso set 1 e poi vinto
    // Se il player Ã¨ loser e w1 > l1 â†’ ha perso set 1 (il winner gli ha preso il set1)
...
/**
 * Calcola ROI con stake fisso (1 unitÃ  per match)
...
function calculateROI(matches, playerName, oddsField = 'odds_b365') {
  ...
  for (const match of matches) {
    ...
    const odds = isWinner ? match[`${oddsField}_winner`] : match[`${oddsField}_loser`];
...
function groupBySurface(matches) {
  ...
  for (const [surface, data] of Object.entries(surfaces)) {
    ...
    for (const match of data.matches) {
      const odds = match.player_role === 'winner' ? match.odds_b365_winner : match.odds_b365_loser;
...
async function getPlayerStats(playerName) {
  console.log(`ðŸ“Š Calculating stats for: ${playerName}`);
  ...
  const avgRanking =
    rankings.length > 0 ? Math.round(rankings.reduce((a, b) => a + b, 0) / rankings.length) : null;
...
async function searchPlayers(query, limit = 10) {
  ...
  // Per ogni giocatore unico, conta wins e losses da matches
  const playerStats = [];
  ...
  const players = playerStats
    .sort((a, b) => {
      ...
      // Poi per numero di match (piÃ¹ dati = piÃ¹ rilevante)
...
async function getHeadToHeadStats(player1, player2) {
  if (!supabase) {
    console.warn('âš ï¸ Supabase not available for H2H');
  ...
  console.log(
    `ðŸŽ¾ [H2H] Searching matches: "${player1}" (${lastName1}) vs "${player2}" (${lastName2})`
  );

  // matches non ha winner_name/loser_name, skip legacy search
  let matches1wins = [];
  let matches2wins = [];

  // ===== STRATEGY: Cerca per player_id in matches =====
  // Trova ID giocatori
  ...
  if (matchingIds1.length > 0 && matchingIds2.length > 0) {
    ...
    const { data: p1p2Matches } = await supabase
      .from('matches')
      .select(
        `*, player1:players!matches_player1_id_fkey(id, name), player2:players!matches_player2_id_fkey(id, name)`
      )
  ...
  // ===== COMBINE E DEDUPLICA =====
  const allMatches = [...(matches1wins || []), ...(matches2wins || []), ...matchesByPlayerId];
  ...
  for (const match of allMatches) {
    ...
    const hasPlayer1 =
      matchingIds1.includes(match.player1_id) || matchingIds1.includes(match.player2_id);

    const hasPlayer2 =
      matchingIds2.includes(match.player1_id) || matchingIds2.includes(match.player2_id);
    ...
    // Check by player_id + winner_code (matches format)
    ...
    // Fallback: check sets won (matches uses sets_player1/sets_player2)
    if (!p1Won && !p2Won) {
      const p1Sets = match.sets_player1 || 0;
      const p2Sets = match.sets_player2 || 0;
      const p1IsPlayer1 = matchingIds1.includes(match.player1_id);
  ...
  console.log(
    `   âœ… [H2H] Found ${h2hMatches.length} matches: ${player1} ${player1Wins} - ${player2Wins} ${player2}`
  );
  ...
  return {
    ...
    player1Wins,
    player2Wins,
    matches: h2hMatches.slice(0, 20), // Limita a 20 match piÃ¹ recenti
  };
}
...
async function calculateSurfaceSplits(playerName, options = {}) {
  ...
  for (const [surface, data] of Object.entries(surfaces)) {
    ...
    if (matchCount >= SURFACE_SAMPLE_THRESHOLDS.MIN_SAMPLE) {
      ...
      holdRate = 0.7 + winRate * 0.2;
      // Break rate typically 15-35% for pros
      breakRate = 0.15 + winRate * 0.2;
    }
    ...
    result[surface] = {
      ...
      recentForm: data.matches
        .slice(-5)
        .map((m) => (m.player_role === 'winner' ? 'W' : 'L'))
        .reverse(),
    };
  }
...
function calculateSurfaceAdvantage(p1Stats, p2Stats) {
  ...
  let advantage = 'even';
  if (diff > 0.1) advantage = 'playerA';
  else if (diff < -0.1) advantage = 'playerB';
