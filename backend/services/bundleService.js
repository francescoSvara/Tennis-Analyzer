...
function loadDependencies() {
  if (!matchRepository) {
    ...
    catch (e) { console.warn('⚠️ matchRepository not available for bundleService'); }
  }
  if (!supabaseClient) {
    try { supabaseClient = require('../db/supabase'); }
    catch (e) { console.warn('⚠️ supabaseClient not available for bundleService'); }
  }
  if (!matchCardService) {
    try { matchCardService = require('./matchCardService'); }
    catch (e) { console.warn('⚠️ matchCardService not available for bundleService'); }
  }
  if (!playerStatsService) {
    try { playerStatsService = require('./playerStatsService'); }
    catch (e) { console.warn('⚠️ playerStatsService not available for bundleService'); }
  }
  if (!featureEngine) {
    try { featureEngine = require('../utils/featureEngine'); }
    catch (e) { console.warn('⚠️ featureEngine not available for bundleService'); }
  }
  if (!strategyEngine) {
    try { strategyEngine = require('../strategies/strategyEngine'); }
    catch (e) { console.warn('⚠️ strategyEngine not available for bundleService'); }
  }
  if (!riskEngine) {
    try { riskEngine = require('./riskEngine'); }
    catch (e) { /* riskEngine optional */ }
  }
  if (!dataQualityChecker) {
    try { dataQualityChecker = require('./dataQualityChecker'); }
    catch (e) { /* dataQualityChecker optional */ }
  }
  if (!liveManager) {
    try { liveManager = require('../liveManager'); }
    catch (e) { console.warn('⚠️ liveManager not available for bundleService'); }
  }
}
...
async function buildBundle(eventId, options = {}) {
  ...
  const needsRefresh = finalMatchData && liveManager && (
  ...
  // 14. Cache bundle
  const isLive = matchStatus === 'inprogress' || matchStatus === 'live' || matchStatus === 'playing';
