 ...
 * Uses matches schema with player1_id/player2_id
 *
...
async function getPlayerMatches(playerName) {
  ...
  // ===== STRATEGY: Search by player_id in matches =====
  // Trova ID giocatori
  ...
  logger.debug(
    `Found ${matchingPlayerIds.length} matching player IDs: ${matchingPlayerIds.join(', ')}`
  );

  // Skip legacy winner_name/loser_name queries (matches doesn't have these)
  let matchesWinner = [];
  let matchesLoser = [];
  const err1 = null;
  const err2 = null;
  ...
  // Query matches by player_id in matches
  ...
  for (const match of allMatches) {
    ...
    // Check by player_id (matches format using player1_id/player2_id)
  ...
  logger.info(
    `Total unique matches found: ${result.length} (${
...
function calculateFirstSetWinRate(matches) {
  ...
  return {
    ...
    match_win_after_first_set:
      matchesWithFirstSetWin > 0 ? matchWinsAfterFirstSet / matchesWithFirstSetWin : 0,
  };
}
...
function calculateDecidingSetWinRate(matches) {
  ...
  for (const match of matches) {
    ...
    const isDecidingSet =
      (format === 'best_of_3' && setsPlayer === 1 && setsOpponent === 1) ||
...
function aggregateBySeries(matches) {
  ...
  for (const series of SERIES) {
    ...
    const finalsReached = seriesMatches.filter(
      (m) => m.round?.toLowerCase().includes('final') && !m.round?.toLowerCase().includes('semi')
    ).length;
...
function analyzeRecentForm(matches, count = 20) {
  ...
  const recentWinRate =
    recent.length > 0 ? recent.filter((m) => m.player_role === 'winner').length / recent.length : 0;
...
async function compareProfiles(player1Name, player2Name, options = {}) {
  ...
  const comparison = {
    ...
    advantages: {
      ...
      surface_win_rate:
        surface && profile1.by_surface[surface] && profile2.by_surface[surface]
          ? {
              leader:
      ...
      recent_form: {
        leader:
          profile1.recent_form.win_rate > profile2.recent_form.win_rate ? player1Name : player2Name,
