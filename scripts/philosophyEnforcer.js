...
function extractRulesFromPseudocode(content, fileName) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const blockPatterns = [
      {
        pattern: /^(RULE|INVARIANT|ASSERT|CHECK|DEFINE|CONST|STRUCT|FUNCTION|FLOW|POLICY)\s+(\w+)/,
        type: 'named',
      },
...
const DB_CHECKS = {
  ...
  DATAQUALITY_BACKEND_ONLY: () => {
    ...
    for (const f of frontendFiles) {
      if (
        fileContains(f, 'calculateDataQuality') ||
  ...
  PBP_ROW_MAPPING: () => {
    ...
    if (/serverIsTop|isHomeTop/.test(content) && /homeScore\s*=\s*serverIsTop/.test(content)) {
      violations.push(
        'sofascoreScraper.js usa logica condizionale per row mapping (DEVE essere fisso: row1=home)'
      );
    }
    return violations.length === 0 ? null : violations;
  },
...
const CALCOLI_CHECKS = {
  ...
  NEVER_RETURN_NULL: () => {
    ...
    if (!content) {
      ...
      if (nullReturns && nullReturns.length > 0) {
        violations.push(
          `featureEngine.js ha ${nullReturns.length} return null/undefined (deve usare fallback)`
        );
      }
    }
    return violations.length === 0 ? null : violations;
  ...
  DOMAIN_MatchState_serverState: () => {
    ...
    for (const f of backendFiles) {
      if (
        fileContains(f, 'serverState') ||
        fileContains(f, 'server') ||
        fileContains(f, 'serving')
      ) {
        found = true;
        break;
      }
  ...
  DOMAIN_BreakServe_serveDominance: () => {
    ...
    for (const f of backendFiles) {
      if (
        fileContains(f, 'serveDominance') ||
        fileContains(f, 'serve_dominance') ||
        fileContains(f, 'dominance')
      ) {
        found = true;
        break;
      }
  ...
  FEATURE_hpi_exists: () => {
    ...
    for (const f of backendFiles) {
      if (
        fileContains(f, 'calculateHPI') ||
        fileContains(f, 'computeHPI') ||
        fileContains(f, 'holdPressureIndex')
      ) {
        found = true;
        break;
      }
...
const STATS_CHECKS = {
  ...
  LEVEL_Player: () => {
    ...
    for (const f of backendFiles) {
      if (
        fileContains(f, 'player_stats') ||
        fileContains(f, 'playerStats') ||
        fileContains(f, 'historical')
      ) {
        found = true;
        break;
      }
  ...
  LEVEL_Match: () => {
    ...
    for (const f of backendFiles) {
      if (
        fileContains(f, 'match_state') ||
        fileContains(f, 'matchState') ||
        fileContains(f, 'current_score')
      ) {
        found = true;
        break;
      }
  ...
  FEATURE_DECLARATION: () => {
    ...
    if (!content) {
      ...
      if (functions.length > 0 && jsdocs.length < functions.length / 2) {
        violations.push(
          `featureEngine.js ha ${functions.length} funzioni ma solo ${jsdocs.length} JSDoc (manca documentazione)`
        );
      }
    }
    return violations.length === 0 ? null : violations;
...
const FRONTEND_DATA_CHECKS = {
    ...
    for (const f of frontendFiles) {
      ...
      if (fallbacks && fallbacks.length > 3) {
        violations.push(
          `${path.basename(f)} ha ${
            fallbacks.length
          } fallback frontend (backend deve sempre fornire valori)`
        );
      }
    }
    return violations.length === 0 ? null : violations;
  ...
  TAB_ISOLATION: () => {
    ...
    for (const f of tabComponents) {
      ...
      if (uniqueTabs.length > 2) {
        violations.push(
          `${path.basename(f)} accede a ${uniqueTabs.length} tabs diverse (violazione isolamento)`
        );
      }
    }
    return violations.length === 0 ? null : violations;
  ...
  FLOW_InitialLoad_skeleton: () => {
    ...
    for (const f of frontendFiles) {
      if (
        fileContains(f, 'skeleton') ||
        fileContains(f, 'Skeleton') ||
        fileContains(f, 'loading')
      ) {
        hasSkeleton = true;
        break;
      }
  ...
  INCREMENTAL_PATCHES: () => {
    ...
    for (const f of hooks) {
      if (
        fileContains(f, 'patch') ||
        fileContains(f, 'Patch') ||
        fileContains(f, 'WebSocket') ||
        fileContains(f, 'ws')
      ) {
        hasPatchLogic = true;
        break;
    ...
    for (const f of frontendFiles) {
      ...
      if (/dataQuality.*if|if.*dataQuality.*then/.test(content.toLowerCase())) {
        ...
        if (/calculateFallback|applyFallback|computeDefault/.test(content)) {
          violations.push(
            `${path.basename(f)} applica fallback logic su dataQuality (deve solo display)`
          );
        }
      }
    }
  ...
  HOOK_useMatchBundle: () => {
    ...
    if (otherDataHooks > 2) {
      violations.push(
        `Ci sono ${otherDataHooks} hooks di fetch separati (deve esserci solo useMatchBundle)`
      );
    }
...
const LINEAGE_CHECKS = {
  ...
  LINEAGE_FLOW: () => {
    const requiredFiles = [
      ...
      {
        path: path.join(BACKEND_DIR, 'services', 'matchCardService.js'),
        step: 'MatchBundleSnapshot',
      },
    ];
    const violations = [];
...
const OBSERVABILITY_CHECKS = {
  ...
  STRUCTURED_LOGGING: () => {
    ...
    if (unstructuredLogs > structuredLogs * 2) {
      violations.push(
        `Troppi log non strutturati: ${unstructuredLogs} vs ${structuredLogs} strutturati`
      );
    }
    return violations.length === 0 ? null : violations;
  },
...
const REGISTRY_CHECKS = {
  ...
  RULE_FK_CANONICAL: () => {
    ...
    for (const m of migrations) {
      ...
      if (!content) continue;
      if (
        /REFERENCES/.test(content) &&
        /REFERENCES.*\(name\)|REFERENCES.*\(raw_name\)/.test(content)
      ) {
...
const LIVE_CHECKS = {
  ...
  RULE_WS_CONTRACT: () => {
    ...
    for (const f of serverFiles) {
      ...
      if (
        content &&
        (content.includes('WebSocket') || content.includes('ws://') || content.includes('socket'))
      ) {
        hasWs = true;
        break;
      }
...
const ODDS_CHECKS = {
    ...
    for (const f of oddsFiles) {
      ...
      if (content.includes('win_probability') || content.includes('winProbability')) {
        if (!content.includes('implied')) {
          violations.push(
            `${path.basename(f)} calcola win_probability (deve calcolare implied_probability)`
          );
        }
      }
    }
  ...
  STRUCT_MarketOdds: () => {
    ...
    for (const f of oddsFiles) {
      ...
      if (
        content &&
        (content.includes('back') || content.includes('lay') || content.includes('odds'))
      ) {
  ...
  CONST_STALENESS_THRESHOLDS: () => {
    ...
    for (const f of oddsFiles) {
      if (
        fileContains(f, 'STALENESS') ||
        fileContains(f, 'stale') ||
        fileContains(f, 'threshold')
      ) {
        return null;
      }
    }
...
const RISK_CHECKS = {
  ...
  FUNCTION_calculatePriceMin: () => {
    ...
    for (const f of backendFiles) {
      if (
        fileContains(f, 'price_min') ||
        fileContains(f, 'priceMin') ||
        fileContains(f, 'minimum.*price')
      ) {
        return null;
      }
    }
...
const CONCEPT_CHECKS_CHECKS = {
  ...
  INVARIANT_SIGNAL_NOT_METRIC: () => {
    ...
    for (const m of migrations) {
      ...
      if (/signal_history|strategy_history|ready_log/.test(content)) {
        violations.push(
          `${path.basename(m)} persiste signals come history (signals are point-in-time)`
        );
      }
    }
    return violations.length === 0 ? null : violations;
  ...
  RULE_NO_FUTURE_DATA: () => {
    ...
    for (const f of repos) {
      ...
      if (
        /SELECT.*FROM.*WHERE/.test(content) &&
        !/event_time|as_of_time|created_at/.test(content)
      ) {
...
const FRONTEND_UI_CHECKS = {
  ...
  RULE_UI_PURPOSE: () => {
    ...
    for (const f of matchPageComponents) {
      ...
      if (
        content &&
        (content.includes('semaphore') || content.includes('READY') || content.includes('status'))
      ) {
  ...
  RULE_RIGHT_RAIL_PURPOSE: () => {
    ...
    for (const f of matchPageComponents) {
      ...
      if (
        content &&
        (content.includes('RightRail') || content.includes('sidebar') || content.includes('sticky'))
      ) {
        hasRightRail = true;
        break;
      }
  ...
  RULE_NO_NULL_DISPLAY: () => {
    ...
    for (const f of frontendFiles) {
      ...
      if (/['"]N\/A['"]|['"]—['"]|['"]null['"]|['"]undefined['"]/i.test(content)) {
        violations.push(
          `${path.basename(f)} mostra N/A o placeholder (backend must provide value)`
        );
      }
    }
    return violations.length === 0 ? null : violations;
...
function runAllChecks() {
  ...
  console.log(
    `${BOLD}${CYAN}║      🧠 PHILOSOPHY ENFORCER v2.0 - Verifica Semantica Completa   ║${RESET}`
  );
  console.log(
    `${BOLD}${CYAN}╚══════════════════════════════════════════════════════════════════╝${RESET}\n`
  );
...
function printFinalReport() {
  ...
  for (const p of results.passed) {
    filosofieStats[p.filosofia] = filosofieStats[p.filosofia] || {
      passed: 0,
      errors: 0,
      warnings: 0,
    };
  ...
  for (const e of results.errors) {
    filosofieStats[e.filosofia] = filosofieStats[e.filosofia] || {
      passed: 0,
      errors: 0,
      warnings: 0,
    };
  ...
  for (const w of results.warnings) {
    filosofieStats[w.filosofia] = filosofieStats[w.filosofia] || {
      passed: 0,
      errors: 0,
      warnings: 0,
    };
  ...
  for (const [filosofia, stats] of Object.entries(filosofieStats)) {
    ...
    console.log(
      `  ${bar}${filosofia}${RESET}: ${stats.passed}/${total} passed (${passPct}%) | ${stats.errors} errors | ${stats.warnings} warnings`
    );
  }
  console.log();
  ...
  if (ciPassed) {
    ...
    console.log(
      `${BOLD}${GREEN}║                    ✓ CI PASSED (con warnings)                    ║${RESET}`
    );
    console.log(
      `${BOLD}${GREEN}╚══════════════════════════════════════════════════════════════════╝${RESET}\n`
    );
    ...
    console.log(
      `${BOLD}${RED}║                    ✗ CI FAILED - Fix errors                      ║${RESET}`
    );
    console.log(
      `${BOLD}${RED}╚══════════════════════════════════════════════════════════════════╝${RESET}\n`
    );
...
> **Philosophy Enforcer v2.0**: ${results.errors.length} errori, ${
...
`;
    ...
    results.errors.forEach((e, i) => {
      md += `- [ ] **ERR-${String(i + 1).padStart(3, '0')}** [${e.filosofia}] ${e.rule}: ${
        e.message
      }\n`;
    });
    md += '\n';
...
`;
    ...
    results.warnings.forEach((w, i) => {
      md += `- [ ] **WARN-${String(i + 1).padStart(3, '0')}** [${w.filosofia}] ${w.rule}: ${
        w.message
      }\n`;
    });
    md += '\n';
