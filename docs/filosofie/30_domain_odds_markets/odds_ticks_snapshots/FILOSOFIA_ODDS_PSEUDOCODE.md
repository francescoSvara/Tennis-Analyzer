...
RULE ODDS_PURPOSE
  ODDS DOES:
    ...
    - provide market features
  
  ODDS DOES NOT:
    - decide stake
    - decide strategies
    - calculate fair odds (that's Predictor)
END
...
REFERENCES
  ...
  DEPENDS_ON:
    - FILOSOFIA_DB (odds_tick, odds_snapshot tables)
    - FILOSOFIA_TEMPORAL (tick timestamps, as_of_time)
    - FILOSOFIA_REGISTRY_CANON (market identifiers)
  OUTPUTS_TO:
    - FILOSOFIA_CALCOLI (implied probabilities, overround)
    - FILOSOFIA_STATS (odds features)
    - FILOSOFIA_LIVE_TRACKING (live odds integration)
    - FILOSOFIA_RISK_BANKROLL (price validation)
    - FILOSOFIA_FRONTEND_DATA_CONSUMPTION (market display)
END
