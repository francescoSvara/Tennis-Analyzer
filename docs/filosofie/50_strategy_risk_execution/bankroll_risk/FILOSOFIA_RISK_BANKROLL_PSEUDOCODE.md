...
f = edge / (price - 1)
stake = bankroll _ f _ kelly_fraction
...
f = 0.06 / 0.85 = 0.0706
stake = 10000 _ 0.0706 _ 0.25 = 176.5
→ bet €176
END
...
Every decision MUST be traceable back to: - Which features were used - What version calculated them - What was the market state - Why this stake was suggested
END
...
TRUTH:
Strategy with positive edge + Wrong staking
= Destroyed bankroll
...
DEPENDS_ON: - FILOSOFIA_DB (bet_decisions persistence) - FILOSOFIA_CALCOLI (edge calculation) - FILOSOFIA_STATS (strategy signals) - FILOSOFIA_ODDS (price validation) - FILOSOFIA_LINEAGE_VERSIONING (decision audit trail)
OUTPUTS_TO: - FILOSOFIA_FRONTEND_DATA_CONSUMPTION (bet recommendations) - FILOSOFIA_FRONTEND (user confirmation UI) - FILOSOFIA_OBSERVABILITY_DATAQUALITY (decision quality metrics)
END
