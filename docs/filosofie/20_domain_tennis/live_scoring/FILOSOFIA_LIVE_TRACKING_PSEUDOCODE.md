...
RULE LIVE_PURPOSE
LIVE DOES: - observe realtime events - update runtime features - regenerate strategy signals - send incremental patches

LIVE DOES NOT: - expose raw data to frontend - decide strategies autonomously - produce new persistent data
END
...
RULE LIVE_OUTPUT
LIVE produces ONLY patches on MatchBundle: - patch on header (score, status) - patch on tabs.\* (features, strategies) - patch on dataQuality
...
LIVE updates these metrics: - freshness: how recent is data - staleness: is data expired - completeness: is something missing
...
DEPENDS_ON: - FILOSOFIA_DB (match_bundle_snapshot persistence) - FILOSOFIA_TEMPORAL (ingestion_time, as_of_time) - FILOSOFIA_PBP_EXTRACTION (live point data) - FILOSOFIA_ODDS (live odds integration)
OUTPUTS_TO: - FILOSOFIA_STATS (live feature updates) - FILOSOFIA_CALCOLI (real-time calculations) - FILOSOFIA_FRONTEND_DATA_CONSUMPTION (WebSocket patches) - FILOSOFIA_RISK_BANKROLL (live exposure updates)
END
