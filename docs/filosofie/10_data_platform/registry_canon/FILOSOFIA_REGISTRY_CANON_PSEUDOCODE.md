...
RULE BUNDLE_CANONICAL_IDS
  bundle.header MUST have:
    - home_player.player_id (canonical)
    - away_player.player_id (canonical)
    - tournament.tournament_id (canonical)
END
...
REFERENCES
  ...
  DEPENDS_ON:
    - FILOSOFIA_DB (players, tournaments tables)
  OUTPUTS_TO:
    - FILOSOFIA_LINEAGE_VERSIONING (entity resolution)
    - FILOSOFIA_OBSERVABILITY_DATAQUALITY (identity warnings)
    - FILOSOFIA_CONCEPT_CHECKS (canonical ID validation)
END
