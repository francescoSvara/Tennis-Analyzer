...
REFERENCES
  ...
  DEPENDS_ON:
    - FILOSOFIA_DB (storage timestamps)
  OUTPUTS_TO:
    - FILOSOFIA_OBSERVABILITY_DATAQUALITY (staleness)
    - FILOSOFIA_LINEAGE_VERSIONING (as_of_time)
    - FILOSOFIA_CALCOLI (feature calculation)
    - FILOSOFIA_LIVE_TRACKING (live freshness)
    - FILOSOFIA_ODDS (odds timestamps)
END
