...
REPRODUCIBLE 100%: - Features deterministiche - Strategy signals (READY/WATCH/OFF) - Odds snapshot

NOT REPRODUCIBLE (accepted): - generated_at - ingestion timing
END
...
MUST answer: - Which features were used? - Which strategy version suggested READY? - Can I regenerate this bundle?
...
DEPENDS_ON: - FILOSOFIA_DB (schema versioning) - FILOSOFIA_TEMPORAL (as_of_time) - FILOSOFIA_REGISTRY_CANON (entity mapping versions)
OUTPUTS_TO: - FILOSOFIA_OBSERVABILITY_DATAQUALITY (version drift alerts) - FILOSOFIA_RISK_BANKROLL (bet decision audit) - FILOSOFIA_CONCEPT_CHECKS (version validation)
END
