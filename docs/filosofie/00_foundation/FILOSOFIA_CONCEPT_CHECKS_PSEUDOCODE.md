...
Every piece of code MUST know: - WHO it is - WHAT it can do - WHAT it must NOT do
END
...
DEPENDS_ON: - FILOSOFIA_DB (schema validation) - FILOSOFIA_TEMPORAL (temporal invariants) - FILOSOFIA_REGISTRY_CANON (identity checks) - FILOSOFIA_LINEAGE_VERSIONING (version checks) - FILOSOFIA_OBSERVABILITY_DATAQUALITY (quality gates)
OUTPUTS_TO: - CI/CD pipeline (automated enforcement) - All code modules (invariant checking)
END
