...
Frontend: - receives ONE payload (MatchBundle) - does NOT know DB, pipeline, sources - does NOT recalculate metrics - does NOT deduce domain logic
...
Frontend RECEIVES patches: - score changes - odds updates - point-by-point append - strategy signals update
END
...
Frontend: - READS bundle.dataQuality - SHOWS badge / warning / tooltip - NEVER applies fallback logic
...
DEPENDS_ON: - FILOSOFIA_DB (match_bundle_snapshot API) - FILOSOFIA_LIVE_TRACKING (WebSocket patches) - FILOSOFIA_STATS (pre-calculated features) - FILOSOFIA_CALCOLI (no frontend calc) - FILOSOFIA_ODDS (market data display) - FILOSOFIA_RISK_BANKROLL (bet recommendations)
OUTPUTS_TO: - FILOSOFIA_FRONTEND (UI rendering)
END
